...
async function generatePDF(data, invNo) {
  ...
  const itemRows = data.items
    ...
    .map((item, idx) => `
      <tr style="background:${idx%2===0?'#1a1a1a':'#161616'}">
        <td style="padding:8px 12px;color:#c8c0b0;font-size:13px;">${idx+1}</td>
        <td style="padding:8px 12px;color:#e0d8c8;font-size:13px;">${item.name}</td>
        <td style="padding:8px 12px;color:#e0d8c8;font-size:13px;text-align:right;">
  ...
  <div id="pdf-receipt" style="
    width:595px; background:#0d0d0d; font-family:'Hind Siliguri','Noto Sans Bengali',sans-serif;
    padding:0; margin:0; position:fixed; top:-9999px; left:-9999px; z-index:-1;
  ">
    <!-- Header -->
    <div style="background:#141414;padding:28px 32px 20px;border-bottom:3px solid #C9A84C;">
      <div style="text-align:center;">
        <div style="font-size:26px;font-weight:900;color:#C9A84C;letter-spacing:1px;
      ...
      <div style="display:flex;justify-content:space-between;margin-top:16px;font-size:11px;color:#807060;">
    ...
    <!-- Customer Info -->
    <div style="margin:20px 24px 0;background:#181818;border:1px solid rgba(201,168,76,0.25);
      border-radius:10px;padding:18px 20px;">
      <div style="font-size:10px;color:#C9A84C;letter-spacing:2px;font-weight:700;margin-bottom:12px;">
      ...
      ${[
        ...
        <div style="display:flex;padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.04);">
          <span style="width:160px;font-size:11px;color:#807060;flex-shrink:0;">${k}:</span>
          <span style="font-size:12px;color:#d8d0c0;">${v}</span>
        </div>`).join('')}
    </div>

    <!-- Order Table -->
    <div style="margin:16px 24px 0;">
      <div style="font-size:10px;color:#C9A84C;letter-spacing:2px;font-weight:700;margin-bottom:10px;">
        অর্ডার বিবরণ / ORDER DETAILS
      </div>
      <table style="width:100%;border-collapse:collapse;border:1px solid rgba(201,168,76,0.2);border-radius:8px;overflow:hidden;">
        <thead>
    ...
    <!-- Total -->
    <div style="margin:14px 24px 0;background:linear-gradient(135deg,#1a1400,#141000);
    ...
    ${data.note ? `
    <!-- Note -->
    <div style="margin:12px 24px 0;padding:12px 16px;background:#181818;border-radius:8px;
      border-left:3px solid #C9A84C;">
      <div style="font-size:10px;color:#807060;margin-bottom:4px;">নোট / Note:</div>
      <div style="font-size:12px;color:#c0b8a8;">${data.note}</div>
    </div>` : ''}

    <!-- Footer -->
    <div style="margin:20px 24px 24px;padding-top:16px;border-top:1px solid rgba(201,168,76,0.2);text-align:center;">
      <div style="font-size:12px;color:#a09080;font-style:italic;margin-bottom:4px;">
        Khalid's Dreams-এ কেনাকাটার জন্য ধন্যবাদ!
      </div>
      <div style="font-size:10px;color:#5a5040;">
